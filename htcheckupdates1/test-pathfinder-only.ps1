# Test Pathfinder detection only
Write-Host "Testing Pathfinder version detection..." -ForegroundColor Cyan
Write-Host ""

# Load configuration
$config = Get-Content -Path ".\products-config.json" -Raw | ConvertFrom-Json
$pathfinderProducts = $config.products | Where-Object { $_.id -like '*pathfinder*' }

if ($pathfinderProducts.Count -eq 0) {
    Write-Host "Pathfinder configurations not found!" -ForegroundColor Red
    exit 1
}

Write-Host "Found $($pathfinderProducts.Count) Pathfinder configurations to test:" -ForegroundColor Green
Write-Host ""

$results = @()

foreach ($product in $pathfinderProducts) {
    ...
    try {
        ...
        if ($htmlContent -match $product.pattern) {
            ...
            $searchPattern = "Pathfinder\s+\d{4}"
...
if ($successCount -eq $totalCount) {
    Write-Host "All Pathfinder configurations are working correctly!" -ForegroundColor Green
} else {
